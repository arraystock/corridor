...
extern void serial_write(); /* write a single character      */
extern int serial_read();   /* read and return a single char */

void idt_set(unsigned char num, unsigned long base) {
  ...
  retry:
    ...
    if (ch == '#') {
      ...
      if (checksum != xmitcsum) {
        ...
        serial_write('-'); /* failed checksum */
      } else {
        serial_write('+'); /* successful transfer */

        /* if a sequence char is present, reply the sequence ID */
        if (buffer[2] == ':') {
          serial_write(buffer[0]);
...
void putpacket(unsigned char *buffer) {
  ...
  do {
    ...
    serial_write('#');
    serial_write(hexchars[checksum >> 4]);
    serial_write(hexchars[checksum % 16]);
...
char *mem2hex(mem, buf, count, may_fault) char *mem;
char *buf;
int count;
int may_fault;
{
...
char *hex2mem(buf, mem, count, may_fault) char *buf;
char *mem;
int count;
int may_fault;
{
...
void set_debug_traps(void) {
  ...
  idt_set(0, (unsigned long)_catchException0);
  idt_set(1, (unsigned long)_catchException1);
  idt_set(3, (unsigned long)_catchException3);
  idt_set(4, (unsigned long)_catchException4);
  idt_set(5, (unsigned long)_catchException5);
  idt_set(6, (unsigned long)_catchException6);
  idt_set(7, (unsigned long)_catchException7);
  idt_set(8, (unsigned long)_catchException8);
  idt_set(9, (unsigned long)_catchException9);
  idt_set(10, (unsigned long)_catchException10);
  idt_set(11, (unsigned long)_catchException11);
  idt_set(12, (unsigned long)_catchException12);
  idt_set(13, (unsigned long)_catchException13);
  idt_set(14, (unsigned long)_catchException14);
