...
void mmap_set(int bit) { mem_map[bit / 32] |= (1 << (bit % 32)); }

// Unset any bit (frame) within the memory map bit array
void mmap_unset(int bit) { mem_map[bit / 32] &= ~(1 << (bit % 32)); }

// Test if any bit (frame) is set within the memory map bit array
int mmap_test(int bit) { return mem_map[bit / 32] & (1 << (bit % 32)); }
...
size_t pmmngr_get_memory_size() { return mem_size; }

uint32_t pmmngr_get_block_count() { return max_blocks; }

uint32_t pmmngr_get_use_block_count() { return used_blocks; }

uint32_t pmmngr_get_free_block_count() { return max_blocks - used_blocks; }

uint32_t pmmngr_get_block_size() { return BLOCK_SIZE; }
